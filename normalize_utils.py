...
REGEX_TELEFONE_BR = re.compile(r"(?:\+?55\s*)?(?:\(?\d{2}\)?\s*)?\d{4,5}[\s.-]?\d{4}")
...
def extrair_telefone(texto: str) -> str | None:
    """Extrai o primeiro telefone brasileiro encontrado no texto bruto."""
    if not texto:
        return None
    for m in REGEX_TELEFONE_BR.finditer(texto):
        bruto = m.group(0)
...
def normalizar_item(item: dict) -> dict:
    ...
    codigo = saida.get("codigo")
    if codigo is not None:
        saida["codigo"] = str(codigo).strip() or None
