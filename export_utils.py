...
def build_excel(rows, matrix, empresas, review, sources):
    ...
    n_cols = 5 + len(empresas)  # Nº Item + Código + Descrição + UF + QTD + empresas
    ...
    def _clean_empresa_display(name: str) -> str:
        if not name:
            return ""
        # prefer basename w/o extensão and replace underscores
        base = os.path.splitext(os.path.basename(name))[0]
        return base.replace("_", " ").strip()

    display_empresas = [_clean_empresa_display(e) for e in empresas]
    headers = ["Nº Item", "Código (PI/NSN/PN)", "Descrição", "UF", "QTD"] + display_empresas
    ...
    for r_idx, row in enumerate(rows, start=3):
        ...
        # ── Código PI/NSN/PN (col 2) ──
        c = ws.cell(row=r_idx, column=2, value=row.get("codigo"))
        c.alignment = center_wrap
        c.border = borda

        # ── Descrição (col 3) ──
        c = ws.cell(row=r_idx, column=3, value=row.get("descricao"))
        ...
        # ── UF (col 4) ──
        # Se nao identificar unidade, usa UN por default
        if unidade is None:
            unidade = "UN"
        c = ws.cell(row=r_idx, column=4, value=unidade)
        c.alignment = center_wrap
        c.border = borda

        # ── QTD (col 5) ──
        c = ws.cell(row=r_idx, column=5, value=quantidade)
        c.alignment = center_wrap
        c.border = borda

        precos_validos = []
        for c_idx, empresa in enumerate(empresas, start=6):
