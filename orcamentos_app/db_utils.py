...
def get_cached_file(conn: sqlite3.Connection, file_id: str):
    ...
    cur = conn.execute(
        "SELECT modified_time, empresa, nome, extraction_version FROM arquivos WHERE file_id = ?", (file_id,)
    )
    row = cur.fetchone()
    if row is None:
        return None
...
def save_extraction(conn: sqlite3.Connection, file_id: str, nome: str, empresa: str,
                     modified_time: str, itens: list) -> None:
    conn.execute("DELETE FROM itens WHERE file_id = ?", (file_id,))
    conn.execute("DELETE FROM arquivos WHERE file_id = ?", (file_id,))
    conn.execute(
        "INSERT INTO arquivos (file_id, nome, empresa, modified_time, extraction_version, processado_em) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (
            file_id,
            nome,
